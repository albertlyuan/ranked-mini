    ...
    // [playername,playerData.elo, playerData.wins, playerData.losses]
        ...
        return(
            <td class="clickable highlights" onClick={goToPlayer}>
                <h3>{player[0]} ({player[2]}-{player[3]}) <img title={getRankFromElo(player[1], player[2], player[3]).split("static/media/")[1].split(".")[0]} class="rankImg" src={getRankFromElo(player[1], player[2], player[3])}/></h3>
                <p>elo: {player[1]}</p>
            </td>
        );
    }
